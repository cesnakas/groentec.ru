		...
		if(!$_this.parent().hasClass("selected")){
			...
			function skuProcessingResult(jsonData){
				...
				if(jsonData[0]["PRODUCT"]["PRICE"]["DISCOUNT_PRICE"]){
					...
					var $productPriceVal = $("<span/>", {class: "priceVal"}).attr('data-price',parseInt(jsonData[0]["PRODUCT"]["PRICE"]["DISCOUNT_PRICE"].replace(/[^0-9]/g, ''))).data('price',parseInt(jsonData[0]["PRODUCT"]["PRICE"]["DISCOUNT_PRICE"].replace(/[^0-9]/g, ''))).html(jsonData[0]["PRODUCT"]["PRICE"]["DISCOUNT_PRICE"]);
				...
				//calculator
				if($("td[data-value=KOLICHESTVO_UPK]").html()) {
					$(".js-item_unit_square").html($("td[data-value=KOLICHESTVO_UPK]").html());
				}
				if($("td[data-value=PRICE_M2]").html()) {
					$(".js__item-price").attr("data-price",$("td[data-value=PRICE_M2").html());
					$(".calculator_value_first").html(BX.Currency.currencyFormat($("td[data-value=PRICE_M2").html(), 'RUB', true));
				}
				$(".qty").val(1);
				$(".js-item_add_summ").html(0);
				var inOneUpak = parseFloat($('.js-item_unit_square').text().replace(',','.'));
				var priceVall = parseFloat($('.js__item-price').data('price'));	
				var dlina = parseInt($(".js-item_dlina").val());
				var shirina = parseInt($(".js-item_shirina").val());
				var zazor = parseInt($(".js-item_zazor").val());
				if (zazor) {
					var pl1Up = (parseFloat($("td[data-value=SHIRINA_MM]").html())+zazor)*parseFloat($("td[data-value=DLINA_MM]").html())/1000000*inOneUpak;
				} else {
					var pl1Up = parseFloat($("td[data-value=SHIRINA_MM]").html())*parseFloat($("td[data-value=DLINA_MM]").html())/1000000*inOneUpak;
				}
				var userField = dlina*shirina/10000;
				if (userField) {
					$('.js-item_square').html(userField.toFixed(2));
				} else {
					$('.js-item_square').html(0);
				}
	
				var countUp = Math.ceil(userField/pl1Up);
				var sumPrice = parseInt(countUp * parseFloat($('#elementTools').find('.priceVal').attr('data-price')));
				$('.qty').each(function() {
					if(countUp) {
						$(this).val(countUp);
						$(".addCart").data("quantity", countUp);
					}else{
						$(this).val(1);
						$(".addCart").data("quantity", 1);
					}
				});
				$('.allSum').each(function() {
					if(sumPrice) {
						$(this).text(''+sumPrice.toFixed(2)+'');
					} else {
						$(this).text(0);
					}
				});
